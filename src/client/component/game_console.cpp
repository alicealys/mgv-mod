...
namespace game_console
{
	namespace
	{
		...
		float color_mgv[4] = {0.75f, 0.59f, 0.4f, 1.f};
		float color_white[4] = {1.f, 1.f, 1.f, 1.f};
		float color_black[4] = {0.f, 0.f, 0.f, 1.f};

		constexpr const auto margin = 4.f;
		constexpr const auto font_height = 10.f;
		constexpr const auto line_height = font_height + 4.f;
		...
		{
			const auto width = renderer::draw_text(instance, "mgv-mod > ", font_height, margin + 2.f, margin + 1.f, color_mgv, color_black);
			return margin + 4.f + width;
		}

		void draw_input_text(game::fox::gr::dg::plugins::Draw2DRenderer* instance, float offset)
		{
			renderer::draw_text_with_cursor(instance, con.input, con.cursor, font_height, offset, margin + 1.f, 
				color_white, color_black, true, 1280.f - offset - margin);
		}
			...
			float* color, float x_offset = 0.f, float y_offset = 0.f, float max_width = 200.f)
		{
			...
			renderer::draw_text(instance, text, font_height, x_offset, y, color, color_black, true, max_width, line_height);
		}
		...
		void draw_hints(game::fox::gr::dg::plugins::Draw2DRenderer* instance, float offset)
		{
			...
			else if (matches.size() == 1)
			{
				...
				if (match.is_var)
				{
					const auto current = match.var->current.to_string();
					const auto reset = match.var->reset.to_string();
					const auto& description = match.var->description;
		...
		void draw_output_text(game::fox::gr::dg::plugins::Draw2DRenderer* instance, output_queue& output)
		{
			...
			for (auto i = 0; i < std::min(static_cast<int>(output.size()), output_line_count); i++)
			{
				...
				renderer::draw_text(instance, output.at(index).data(), font_height, margin + 2.f, y + offset, color_white, color_black, true, 1265.f - margin * 2.f, line_height);
				y += line_height;
			}
		}
	...
	public:
		void pre_load() override
		{
			...
			var_con_output_slider_color = vars::register_color(
				...
				{color_mgv[0], color_mgv[1], color_mgv[2], color_mgv[3]},
				vars::var_flag_saved,
