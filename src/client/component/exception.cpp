...
namespace exception
{
	namespace
	{
		...
		void display_error_dialog()
		{
			...
			const auto game_module = GetModuleHandle(NULL);
			std::size_t address{};
			if (has_module && module == game_module)
			{
				address = (reinterpret_cast<std::size_t>(exception_data.address) -
					reinterpret_cast<std::size_t>(game_module)) + 0x140000000;
			}
			else
			{
				address = reinterpret_cast<std::size_t>(exception_data.address);
		...
		std::string generate_crash_info(const LPEXCEPTION_POINTERS exception_info)
		{
			...
			line(utils::string::va("Base address: 0x%llX", game::get_base_address()));
	...
	public:
		void pre_load() override
		{
			...
			utils::hook::set(0x141FEA270_r, set_unhandled_exception_filter_stub);
			utils::hook::set(0x141FEA7D8_r, set_unhandled_exception_filter_stub);
			utils::hook::set<std::uint8_t>(0x14196E270_r, 0xC3);
